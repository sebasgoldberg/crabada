...
export class CrabadaAPI{
    ...
    async getClosedMines(page: number, limit: number): Promise<IApiMine[]>{
        const headers = {
            ...
            authorization: `Bearer ${env.CONSTANT_TOKEN}`,
            'sec-ch-ua-mobile': '?0',
        ...
        DEBUG && console.log('GET', `${this.idleGameApiBaseUrl}/public/idle/mines?page=${page}&limit=${limit}&status=close`);
        ...
        const { 
            result: {
                ...
                `${this.idleGameApiBaseUrl}/public/idle/mines?page=${page}&limit=${limit}&status=close`,
                {
                    headers
                })
            ).data
...
export const listenCanLootGamesFromApi = async (
    ...
    let actualPage = 0
    const minesPerPage = 15
    let readOnlyFirstPage = READ_ONLY_FIRST_PAGE
    ...
    return setInterval(async () => {
        ...
        try {
            ...
            actualPage = readOnlyFirstPage ? 1 : actualPage+1
