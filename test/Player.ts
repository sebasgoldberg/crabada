...
describe('Win-Win Strategy', function () {
  ...
  describe('Player', function () {
    ...
    it('should deposit work as expected using external account.', async function () {
    
      const player1 = this.player1 as Contract
  
      await this.IdleGame.connect(this.owner).deposit(crabada)
  
      for (const c of crabada){
        expect(await this.IdleGame.ownerOf(c)).to.eq(accounts.owner)
      }
  
    });
  
  
    it('should be possible to deposit crabadas using player contract.', async function () {
      ...
      for (const c of crabada){
        expect(await this.IdleGame.ownerOf(c)).to.eq(player1.address)
      }
  
    });
  
  
    it('should be possible to create a team using player contract.', async function () {
      ...
      expect(await player1.teamsCount()).equals(1)
  
      const team1 = await player1.teams(0)
  
      const teamInfo = await this.IdleGame.getTeamInfo(team1)
      const { crabadaId1: c1t1, crabadaId2: c2t1, crabadaId3: c3t1 } = teamInfo
  
      const t1ExpectedMembers = this.team1Members.map( (x: BigNumber) => x.toNumber())
      const t1Members = [c1t1, c2t1, c3t1].map( (x: BigNumber) => x.toNumber())
  
      t1Members.forEach( 
        m => expect(t1ExpectedMembers.indexOf(m)).greaterThanOrEqual(0)
      )
  
    });
  
  
    it('should be possible to start a game.', async function () {
    ...
    it('should be possible to start and close it.', async function () {
      ...
      await this.IdleGame.connect(account0).closeGame(currentGameId)
  
      const tusFinalBalance: BigNumber = await this.tusToken.balanceOf(player1.address)
      const craFinalBalance: BigNumber = await this.craToken.balanceOf(player1.address)
    ...
    it('should be possible to attack.', async function () {
      ...
      const team2 = await player2.teams(0)
    ...
    it('should be possible to attack, close the game, and win all the rewards.', async function () {
      ...
      const teamInfo1 = await this.IdleGame.getTeamInfo(team1)
    ...
    it('should be possible to create a win loop.', async function () {
